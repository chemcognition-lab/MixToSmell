...
from mix2smell.data.featurization import UNK_TOKEN
...
def load_and_freeze_base(model, checkpoint_path):
    state_dict = torch.load(checkpoint_path)
    model.base_layer.load_state_dict({k.replace('output_layer.', ''): v for k, v in state_dict.items() if 'output_layer' in k})
